			...
			if(Inst.StartVehicle[v] >= Inst.Np || Inst.Prod_av[Inst.StartVehicle[v]][t]){
				...
				if(tot>epsi){
					...
					if(Inst.AddImprove==1 && (resV2[0]!=-2 || resV2[1]!=-2)){
						...
						}else if((sol ==-1 && resV2[0]+resV2[1]>MasterCplex.getValue(sigma[t][v])+epsi) || (sol!=-1 && resV2[0]+resV2[1]>MasterCplex.getValue(sigma[t][v],0)+epsi)){
							AddImprovedProdOptCut(env,Inst,Inst.StartVehicle[v],y,sigma,resV2[0]+resV2[1],PickAndDel.first,PickAndDel.second,AddCuts,t,yr);
						}
				...
				if(SigmaHub<resHub[h]){
					if(VarDeliHub[h].size()>0){		
						...
						if(Inst.Bapcod>=1){
							...
							assert(opt1==-1 || (accumulate(TourCostDeli.begin(), TourCostDeli.end(), 0)-opt1<0.1 && accumulate(TourCostDeli.begin(), TourCostDeli.end(), 0)-opt1>-0.1 ));
					...
					if(VarPickHub[h].size()>0){		
						...
						for (size_t i = 0; i < VarPickHub[h].size(); i++){
							...
							demandepernode[ProblemHubPick[h].back()+1]+=DemandsProblemHubPick[h].back();
							if(demandepernode[ProblemHubPick[h].back()+1]>Inst.CapaHub){
								EasySolHigh=false;
						...
						if(Inst.Bapcod>=1){
								...
								if(!EasySolLow && Inst.YannickT==1){
									...
									yCoord.push_back(Inst.y_all[Inst.StartVehicle[Inst.Np+h]]);
									DistDepot.push_back(0);
									demandbap.push_back(0);
									nodeMaster.push_back(Inst.StartVehicle[Inst.Np+h]);
									splitted.push_back(false);
									posinPick.push_back(0);
									for (int i = 0; i < Inst.Np +Inst.Nh; i++){
										if(demandepernode[i+1]>0 && i!= Inst.StartVehicle[Inst.Np+h]){
											if(split[i]){
												xCoord.push_back(Inst.x_all[i]);
												yCoord.push_back(Inst.y_all[i]);
												nodeMaster.push_back(i);
												DistDepot.push_back(Inst.dist[Inst.StartVehicle[Inst.Np+h]][i]);
												demandbap.push_back(demandepernode[i+1]);
												splitted.push_back(false);
												posinPick.push_back(0);
											}else{
												assert(demandepernode[i+1]>Inst.CapaHub);
												for (int j = 0; j < (int) ProblemHubPick[h].size(); j++){
													if(ProblemHubPick[h][j]==i){
														...
														DistDepot.push_back(Inst.dist[Inst.StartVehicle[Inst.Np+h]][ProblemHubPick[h][j]]);
								...
								if(!EasySolLow && Inst.YannickT==1 && EasySolHigh  && (float)accumulate(demandepernode.begin(), demandepernode.end(), 0)/Inst.CapaHub > demandepernode.size()-2){
									for (int i = 1; i < (int)ProblemHubPick[h].size(); i++){
										if(ProblemHubPick[h][i]!=ProblemHubPick[h][i-1]){
											TourPick.push_back({i});
											TourCostPick.push_back(2*Inst.dist[Inst.StartVehicle[Inst.Np+h]][ProblemHubPick[h][i]]);
											opt2+=2*Inst.dist[Inst.StartVehicle[Inst.Np+h]][ProblemHubPick[h][i]];
										}
										else
											TourPick.back().push_back(i);
									}
								}else
									EasySolHigh=false;
								if(!EasySolHigh){
								...
								cout<<opt2<<endl;*/
								assert(opt2==-1 || (accumulate(TourCostPick.begin(), TourCostPick.end(), 0)-opt2<0.1 && accumulate(TourCostPick.begin(), TourCostPick.end(), 0)-opt2>-0.1));								
							#endif
						}else{
							IloEnv env2;	
							IloArray<IloArray<IloNumVar>> u2(env, Inst.node);
...
int mainBend(MyInstance Inst, int BestUpper)
{
	try {
		...
		while(iter < maxIterations && !GetOut){
			...
			MasterCplex.setParam(IloCplex::Param::TimeLimit, max(0.0,Inst.TimeLimit-Inst.SubSolving.count()-Inst.MasterSolving.count()));
			...
			if((upper - lower) / upper < epsi && (Inst.Gap==0 || Inst.Gap>=5)){
				...
				Inst.Gap=1;
				MasterCplex.setParam(IloCplex::Param::MIP::Tolerances::MIPGap, 0.05);
			}else if(Inst.Gap==3 && (upper - lower) / upper< 0.2){
				Inst.Gap=2;
				MasterCplex.setParam(IloCplex::Param::MIP::Tolerances::MIPGap, 0.1);
			}else if(Inst.Gap==4){
				Inst.Gap=3;
				MasterCplex.setParam(IloCplex::Param::MIP::Tolerances::MIPGap, 0.2);
			}else if(Inst.Gap>=5){
